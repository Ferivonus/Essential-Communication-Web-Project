...
// Handler function to reset 'messages_send_to_user_client' table
#[post("/reset-user-client-messages-table")]
pub async fn reset_messages_send_to_user_client_table_handler(
    pool: web::Data<AppState>,
) -> impl Responder {
    let drop_table_query = "DROP TABLE IF EXISTS messages_send_to_user_client;";
    let create_table_query = "
        CREATE TABLE messages_send_to_user_client (
    ...
    match query(drop_table_query).execute(&pool.db).await {
        Ok(_) => match query(create_table_query).execute(&pool.db).await {
            Ok(_) => {
                HttpResponse::Ok().body("Table 'messages_send_to_user_client' reset successfully")
            }
            Err(e) => {
                eprintln!("Error creating 'messages_send_to_user_client' table: {}", e);
                HttpResponse::InternalServerError().body("Error creating table")
            }
        },
        Err(e) => {
            eprintln!("Error dropping 'messages_send_to_user_client' table: {}", e);
